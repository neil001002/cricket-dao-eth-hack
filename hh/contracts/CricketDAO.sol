...
contract CricketDAO is IERC721Receiver {
    INFTMarketplace nftMarketplace;
    ICricketDaoNFT cricketDaoNft;

    constructor(address nftContract, address marketplaceContract) payable {
        cricketDaoNft = ICricketDaoNFT(nftContract);
        nftMarketplace = INFTMarketplace(marketplaceContract);
    }
    ...
    function onERC721Received(
        ...
        require(cricketDaoNft.ownerOf(tokenId) == address(this), "MALICIOUS");
    ...
    function quit() external memberOnly {
        ...
        for (uint256 i = 0; i < member.lockedUpNFTs.length; i++) {
            cricketDaoNft.safeTransferFrom(
                address(this),
                msg.sender,
